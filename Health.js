...
category:PropTypes.string
  ...
  async componentDidMount() {
    console.log("render")
    let url = `https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=51b657d64c4f4b7a9632a19f75e19e8c&pageSize=${this.props.pageSize}`
    this.setState({ loading: true })
    let data = await fetch(url);
    let parseData = await data.json()
    console.log(data)
...
this.setState({ articles:this.state.articles.concat (parseData.articles), totalResults: parseData.totalResults })
  ...
  //   //   let url = `https://newsapi.org/v2/top-headlines?country=in&category=health&apiKey=51b657d64c4f4b7a9632a19f75e19e8c&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`
  //   //   this.setState({ loading: true })
  //   //   let data = await fetch(url);
  //   //   let parseData = await data.json();
  //   //   this.setState({ loading: false })
    ...
    return (
      <>
        ...
        <div className="row">
          ...
          loader={<Spinerr/>}
        > 
          <h1 className='text-center'>   NEWS APP- Top Headlines of Health</h1>
